<?php 
		...
		// assigns value from the edit officer credentials
		if (isset($_POST['UpdateCredentials'])) {
            ...
            echo $user;
			// performs a query
			$result = mysqli_query($conn, "SELECT * FROM officer WHERE username='$oldUser'    ");
			...
			if ($oldUser == $res['username']) {
				...
				if ($oldPass == $res['password']) {
					if ($newPass == "" && $rePass == "") {
						$result = mysqli_query($conn, "UPDATE officer SET username = '$user' WHERE username='$oldUser'");
					...
					// checks if the new pass and and re enter password is the same and performs the query
						...
						$result = mysqli_query($conn, "UPDATE officer SET username = '$user', password = '$rePass' WHERE username='$oldUser'");
